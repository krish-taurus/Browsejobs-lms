...
<head>
<meta charset="utf-8">
<title>Tax Invoice {{ $receipt->number }}</title>
<style>
...
<div class="doc">
  <div class="head">
    ...
    <div style="text-align:right;">
      ...
      <div class="mono" style="font-size:18px; font-weight:700;">{{ $receipt->number }}</div>
      <div class="muted mono">{{ $receipt->created_at?->format('d M Y') }}</div>
    </div>
  </div>
  ...
  <div class="meta">
    ...
    <div>
      <div class="kicker" style="color:var(--muted);">Programme</div>
      <div style="font-weight:600;">{{ $courseName }}</div>
      <div class="muted">Registration · instalment {{ $seq }}</div>
    </div>
  </div>
