 ...
 * review for it is already pending or has already been given.
 */
final readonly class SendReviewRequest
{
    public function __construct(private Messenger $messenger) {}

    public function handle(User $student, ReviewStage $stage, ?Batch $batch = null, ?string $courseName = null): bool
    {
        if (! config('vouchers.review_request.enabled', true)) {
            return false;
        }

        return app(TenantContext::class)->run($student->tenant, function () use ($student, $stage, $batch, $courseName): bool {
